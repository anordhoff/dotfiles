...
" TODO: look into nvim-lint and golangci-lint
...
" NOTE: vim-sleuth sets softtabstop to -1
set tabstop=2
set shiftwidth=2
set softtabstop=0
set expandtab
...
" make <left> and <right> move cursor instead of selecting wildmenu matches
cnoremap <left> <space><bs><left>
cnoremap <right> <space><bs><right>

" make [[, ][, ]], and [] useful (overwrites existing search) (see :h section)
...
" TODO: is this needed now that dispatch does the same thing?
" quickly run a command in a new terminal buffer
" nnoremap z<space> :Term<space>
" command! -nargs=1 Term call s:Terminal(<q-args>)
...
function QuickfixList()
  if getqflist({'winid': 0}).winid
    ...
    " TODO: botright loclist gets moved around
    " prevent location list from growing vertically
    " for w in range(1, winnr('$'))
    "   let id = win_getid(w)
    "   if getloclist(id, {'winid': 0}).winid
    "     call win_execute(id, 'noautocmd lclose | noautocmd lopen')
    "   endif
    " endfor
  else
    botright copen
...
" TODO: use ctrl-i to insert tab character
"       not supported by tmux (https://github.com/tmux/tmux/issues/2705)
if $TERM ==# 'xterm-kitty'
